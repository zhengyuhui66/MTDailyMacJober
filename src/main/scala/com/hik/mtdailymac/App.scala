...
/**
  ...
  * 依据次数和天数的阈值来判断此Mac用户是否为常住用户
  */
object App{

  val MACRECORDER_CONF:String="dataAnalysis/mtdailymac.xml"
  ...
  def main(args: Array[String]): Unit = {
    ...
    hBaseUtil.deleteTable(configUtil.mtDailyMac)
    hBaseUtil.deleteTable(configUtil.mtDailyWorkMac)
    hBaseUtil.deleteTable(configUtil.mtDailyRestMac)
    hBaseUtil.deleteTable(configUtil.mtDailyMacCountByDevId)

    hBaseUtil.createTable(configUtil.mtDailyMac, "DL");
    hBaseUtil.createTable(configUtil.mtDailyWorkMac, "DL");
    hBaseUtil.createTable(configUtil.mtDailyRestMac, "DL");
    hBaseUtil.createTable(configUtil.mtDailyMacCountByDevId, "DL");
    val broadList = sc.broadcast(List(
      configUtil.confPath,configUtil.recoderTable,
      configUtil.mtDailyMac,configUtil.mtDailyWorkMac,
      configUtil.mtDailyRestMac,configUtil.mtDailyMacCountByDevId,
  ...
  def exeuteRDD(_DataRDD: RDD[(String,Int)],broadList:Broadcast[List[String]]): RDD[(String,String)] = {
    ...
    _DataRDD.reduceByKey(_ + _)
      ...
      val daysThreshold = broadList.value(6).toInt
      val cotsThreshold = broadList.value(7).toInt
      println("daysThreshold:"+daysThreshold+"  cotsThreshold:"+cotsThreshold+"  x._2._1:"+x._2._1 +"  x._2._2:"+x._2._2)
      ...
      val HASH_DEVID=CommFunUtils.byte2HexStr(CommFunUtils.GetHashCodeWithLimit(devid, 0xFF).toByte).toUpperCase
      val nj = new JSONObject()
      nj.accumulate("mac", mac)
      nj.accumulate("day", days)
      nj.accumulate("cot", cots)
      val macs=mac.replace("-", "")
      (HASH_DEVID+devid+macs,nj.toString)
    })
  }
